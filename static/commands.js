 ...
 *
 * Tries the real steer endpoint first. On any non-accept response, keeps the
 * active stream running. Normal composer use restores the draft; queued-steer
 * use requeues the removed queue entry.
 *
 * @param {string} msg - The steer text.
 * @param {boolean} explicitSteer - True if the user explicitly invoked /steer.
 * @param {object=} fallbackPayload - Queue payload to restore if a queued steer fails.
 * @returns {Promise<boolean>} true when steer was delivered.
 */
    ...
    api('/api/reasoning',{method:'POST',body:JSON.stringify({effort:arg})})
      .then(function(st){
        ...
        if(typeof _applyReasoningChip==='function'){
          const override=(
            typeof _currentReasoningSessionOverride!=='undefined' &&
            _currentReasoningSessionOverride
          )||null;
          _applyReasoningChip(override||eff,override);
        }
      })
      .catch(function(e){
