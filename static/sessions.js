...
// Expands S.messages to a larger authoritative tail window and re-renders,
// preserving scroll position as older messages appear above the viewport.
...
function renderSessionListFromCache(){
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    el.className='session-item'+(isActive?' active':'')+(isActive&&S.session&&S.session._flash?' new-flash':'')+(s.archived?' archived':'')+(isStreaming?' streaming':'')+(hasUnread?' unread':'')+(attention?' needs-attention':'')+attentionClass+(isTitleGenerating?' title-generating':'');
...
async function deleteSession(sid, beforeDelete=null){
  ...
  // Queue is backend-canonical in this fork: clear the local optimistic copy.
  if(typeof _setSessionQueue==='function') _setSessionQueue(sid,[]);
  if(!optimisticRendered){
    _pendingSessionReflowPositions=reflowPositions;
    _optimisticallyRemoveSessionFromList(sid);
  }
