...
async function send(){
  ...
  if(S.busy||compressionRunning){
    if(text||S.pendingFiles.length){
      ...
      // /queue, /terminal, /goal, /background, /btw, or /yolo while the agent is
      // running and have them execute immediately.
      ...
      if(text.startsWith('/')&&!literalSlash){
        ...
        const _immediateBusyCommands=new Set(['steer','interrupt','queue','terminal','goal','yolo','background','btw']);
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('subagent_event',e=>{
      ...
      try{
        const d=JSON.parse(e.data);
        if(typeof updateDelegationPanel==='function') updateDelegationPanel(d);
      }catch(_){}
    });
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        if(isActiveSession){
          ...
          else if(typeof _messageRenderableMessageCount==='function'&&typeof _messageRenderWindowSize!=='undefined'){
          ...
          // server-side reasoning config without changing model/provider cache key.
    ...
    source.addEventListener('pending_steer_leftover',e=>{
      ...
      try{
        ...
        if(typeof clearPendingSteerIndicators==='function') clearPendingSteerIndicators();
        if(typeof _applyToAnchor==='function') _applyToAnchor('pending_steer_leftover',d,e);
    ...
    for(const _runJournalEventName of ['token','interim_assistant','reasoning','tool','tool_complete','subagent_event','todo_state','approval','clarify','state_saved','title','title_status','context_status','goal','goal_continue','done','stream_end','pending_steer_leftover','compressing','compressed','metering','apperror','warning','error','cancel']){
