...
def _split_model_context_for_display(messages):
    ...
    Context engines may inject model-facing guidance by returning system
    messages when WebUI provides a synthetic system anchor. Keep that guidance
    in context_messages, but do not render any system message as a transcript
    turn.
    """
...
def _start_chat_stream_for_session(
    ...
    *,
    ...
    # The check and reservation must stay under the per-session lock: two
    # concurrent /api/chat/start requests must not both observe an idle session
    # and spawn duplicate workers for the same visible turn.
    ...
    while True:
        ...
        with session_lock:
            ...
            if not stale_stream_id:
                ...
                # treated as goal-related. Only consume it once this request owns
                # the new stream.
                if not goal_related and s.session_id in PENDING_GOAL_CONTINUATION:
                    goal_related = True
                    PENDING_GOAL_CONTINUATION.discard(s.session_id)

                # process_complete wakeup marker is server-internal telemetry;
                # discard it atomically once a new stream is claimed for the session.
