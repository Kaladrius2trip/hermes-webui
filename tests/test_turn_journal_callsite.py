...
def test_chat_start_writes_turn_journal_after_session_lock_and_handles_failure():
    src = Path("api/routes.py").read_text(encoding="utf-8")
    chat_start_idx = src.index("def _start_chat_stream_for_session")
    lock_idx = src.index("with session_lock:", chat_start_idx)
    prepare_idx = src.index("_prepare_chat_start_session_for_stream(", lock_idx)
    ...
    assert lock_idx < prepare_idx < append_idx < thread_idx
