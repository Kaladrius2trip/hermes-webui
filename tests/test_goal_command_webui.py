...
def test_frontend_has_goal_slash_command_and_status_event_handler():
    ...
    assert "_immediateBusyCommands" in MESSAGES_JS
    for name in ("steer", "interrupt", "queue", "terminal", "goal"):
        assert f"'{name}'" in MESSAGES_JS
    assert "_immediateBusyCommands.has(_bc.name)" in MESSAGES_JS
