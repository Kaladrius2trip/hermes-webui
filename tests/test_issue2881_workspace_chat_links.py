...
@pytest.mark.parametrize(
    ...
    [
        ...
        "workspace://C:/Users/alice/.ssh/id_rsa",
        "workspace://.env",
        "workspace://.ssh/id_rsa",
        "workspace://notes/http://example.test/plan.md",
    ],
)
def test_render_md_blocks_unsafe_workspace_links(driver_path, target):
    html = _render(driver_path, f"[Open]({target})")
...
def test_streaming_markdown_rewrites_workspace_links_before_sanitizing():
    ...
    assert "'#workspace='" in MESSAGES_JS
    assert "_smdWorkspaceRelFromHref" in MESSAGES_JS
    assert "_WORKSPACE_SECRET_PATH_RE" in MESSAGES_JS
