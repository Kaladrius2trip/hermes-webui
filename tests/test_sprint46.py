...
def test_split_model_context_for_display_keeps_system_messages_model_only():
    import api.routes as routes

    context = [
        {"role": "system", "content": "[LCM retrieval policy: Use lcm_grep.]"},
        {"role": "system", "content": "Active workspace at session start: /private/workspace"},
        {"role": "assistant", "content": "[Recent Summary (d0, node 7)] old context"},
        {"role": "user", "content": "latest"},
    ]

    display, model_context = routes._split_model_context_for_display(context)

    assert model_context == context
    assert display == context[2:]
    assert all(message.get("role") != "system" for message in display)
    assert "Active workspace at session start" not in json.dumps(display)
...
def test_session_compress_keeps_engine_system_guidance_model_facing_only(monkeypatch, cleanup_test_sessions):
    ...
    assert "Active workspace at session start" not in json.dumps(payload["session"]["messages"])


def test_session_compress_strips_injected_lcm_and_anchor_systems_from_display(monkeypatch, cleanup_test_sessions):
    class AnchorOnlyLcmCompressor:
        name = "lcm"
    ...
    class AnchorOnlyLcmAgent:
        def __init__(self, **kwargs):
            self.context_compressor = AnchorOnlyLcmCompressor()

    created = cleanup_test_sessions
    sid = _make_session()
    created.append(sid)
    _install_fake_compression_runtime(monkeypatch, AnchorOnlyLcmAgent)
    ...
    assert "Active workspace at session start" in loaded.context_messages[1]["content"]
    payload_messages = handler.payload()["session"]["messages"]
    assert all(m.get("role") != "system" for m in payload_messages)
    assert "Active workspace at session start" not in json.dumps(payload_messages)
